    ...
    class << self
      ...
      def authenticate(controller)
        controller.authenticate_or_request_with_http_basic('Documentation') do |username, password|
          user = RailsMap::User.find_by(username: username)
      ...
      def authenticate_with_session(controller)
        if controller.session[:rails_map_user_id].present?
          user = RailsMap::User.find_by(id: controller.session[:rails_map_user_id])
          return true if user.present?
        end
        
        controller.redirect_to rails_map.login_path
      end
    end
  end
end
